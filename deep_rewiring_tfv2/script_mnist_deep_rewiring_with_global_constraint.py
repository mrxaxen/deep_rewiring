...
tf.compat.v1.app.flags.DEFINE_float('lr_epoch_decay', 0.8, 'Learning rate decay')
...
decay_lr_op = tf.compat.v1.assign(lr, lr * FLAGS.lr_epoch_decay)
...
def sample_matrix_specific_reconnection_number_for_global_fixed_connectivity(theta_list, ps, upper_bound_check=False):
    with tf.compat.v1.name_scope('NBreconnectGenerator'):
        theta_vals = [theta.read_value() for theta in theta_list]

        # Compute size and probability of connections
        nb_possible_connections_list = [tf.cast(tf.size(input=th), dtype=tf.float32) * p for th, p in zip(theta_list, ps)]
        total_possible_connections = tf.reduce_sum(input_tensor=nb_possible_connections_list)
        ...
        total_connected = tf.reduce_sum(input_tensor=[nb_connected(theta) for theta in theta_vals])
        ...
        with tf.control_dependencies([assert_upper_bound_check]):
            ...
            sample_split = tf.compat.v1.distributions.Categorical(probs=sampling_probs).sample(nb_reconnect)
            is_class_i_list = [tf.equal(sample_split, i) for i in range(len(theta_list))]
            counts = [tf.reduce_sum(input_tensor=tf.cast(is_class_i, dtype=tf.int32)) for is_class_i in is_class_i_list]
            return counts
