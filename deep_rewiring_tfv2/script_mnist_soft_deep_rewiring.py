...
tf.compat.v1.app.flags.DEFINE_integer('batch', 10, 'Batch size')
tf.compat.v1.app.flags.DEFINE_integer('print_every', 500, 'Print every')
#
tf.compat.v1.app.flags.DEFINE_float('p01', .005, 'Proportion of connected synpases at initialization')
tf.compat.v1.app.flags.DEFINE_float('p02', .005, 'Proportion of connected synpases at initialization')
tf.compat.v1.app.flags.DEFINE_float('p0out', .1, 'Proportion of connected synpases at initialization')
tf.compat.v1.app.flags.DEFINE_float('l1', 1e-5, 'L1 reg coeffcient')
tf.compat.v1.app.flags.DEFINE_float('gdnoise', 1e-5, 'Noise amplitude')
tf.compat.v1.app.flags.DEFINE_float('lr', 0.05, 'Learning rate')
#
tf.compat.v1.app.flags.DEFINE_string('comment', '', 'Comment to retrieve the results faster')
...
def weight_sampler_with_clip(n_in, n_out, p0, l1, noise, lr):
    with tf.compat.v1.name_scope('SynapticSampler'):
        ...
        w = tf.compat.v1.where(condition=is_connected, x=w_sign * th, y=tf.zeros((n_in, n_out), dtype=tf.float32))
...
with tf.compat.v1.name_scope('Regularization'):
    apply_grads = tf.compat.v1.train.GradientDescentOptimizer(lr).minimize(cross_entropy)

    connected_mask = lambda th: tf.cast(tf.greater(th.read_value(), 0), tf.float32)
    l1_noisy_update = lambda th: tf.random.normal(shape=tf.shape(input=th), stddev=FLAGS.gdnoise,
    ...
    with tf.control_dependencies([apply_grads]):
        ...
        updates = [tf.compat.v1.assign(th,new_th) for th,new_th in zip(theta_list,new_ths)]
        with tf.control_dependencies(updates):
            train_step = tf.no_op('Train')

with tf.compat.v1.name_scope('Stats'):
    nb_zeros = [tf.reduce_sum(input_tensor=tf.cast(tf.equal(w, 0), dtype)) for w in weight_lists]
    sizes = [tf.cast(tf.size(input=w), dtype=dtype) for w in weight_lists]
