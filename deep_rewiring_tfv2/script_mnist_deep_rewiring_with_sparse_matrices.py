...
tf.compat.v1.app.flags.DEFINE_integer('n_epochs', 10, 'number of iteration (55000 is one epoch)')
tf.compat.v1.app.flags.DEFINE_integer('batch', 10, 'number of iteration (55000 is one epoch)')
tf.compat.v1.app.flags.DEFINE_integer('print_every', 100, 'print every k steps')
tf.compat.v1.app.flags.DEFINE_integer('n1', 300, 'Number of neurons in the first hidden layer')
tf.compat.v1.app.flags.DEFINE_integer('n2', 100, 'Number of neurons in the second hidden layer')
#
tf.compat.v1.app.flags.DEFINE_float('p01', .01, 'Proportion of connected synpases at initialization')
tf.compat.v1.app.flags.DEFINE_float('p02', .03, 'Proportion of connected synpases at initialization')
tf.compat.v1.app.flags.DEFINE_float('p0out', .3, 'Proportion of connected synpases at initialization')
tf.compat.v1.app.flags.DEFINE_float('l1', 1e-5, 'l1 regularization coefficient')
tf.compat.v1.app.flags.DEFINE_float('gdnoise', 1e-5, 'gradient noise coefficient')
tf.compat.v1.app.flags.DEFINE_float('lr', 0.5, 'Learning rate')
...
def weight_sampler_strict_number(n_in, n_out, nb_non_zero, dtype=tf.float32):
    ...
    with tf.compat.v1.name_scope('SynapticSampler'):
        ...
        ind_in = tf.random.uniform(shape=[nb_non_zero],maxval=n_in,dtype=tf.int64)
        ind_out = tf.random.uniform(shape=[nb_non_zero],maxval=n_out,dtype=tf.int64)
...
def rewiring(theta, sign, indices,w):
    ...
    with tf.compat.v1.name_scope('rewiring'):
        ...
        target_nb_connection = tf.size(input=theta)

        #
        reconnect_mask = tf.less_equal(th,0)

        # Gererate the random mask
        ind_in = tf.random.uniform(shape=[target_nb_connection],maxval=n_in,dtype=tf.int64)
        ind_out = tf.random.uniform(shape=[target_nb_connection],maxval=n_out,dtype=tf.int64)
        new_indices = tf.stack([ind_in,ind_out],axis=1)
        new_indices = tf.compat.v1.where(reconnect_mask,new_indices,indices)
        set_indices = tf.compat.v1.assign(indices,new_indices)

        # Generate random signs
        new_sign = tf.random.uniform(shape=[target_nb_connection],maxval=2,dtype=tf.int32) * 2 -1
        new_sign = tf.cast(new_sign,dtype=tf.float32)
        new_sign = tf.compat.v1.where(reconnect_mask, new_sign, sign)
        set_sign = tf.compat.v1.assign(sign, new_sign)

        #new_th = tf.zeros(shape=[target_nb_connection],dtype=tf.float32)
        new_th = tf.random.normal(shape=[target_nb_connection],dtype=tf.float32,stddev=0)
        new_th = tf.compat.v1.where(reconnect_mask,new_th,th)
        set_th = tf.compat.v1.assign(theta,new_th)
...
with tf.compat.v1.name_scope('Layer1'):
    ...
    a_1 = tf.sparse.sparse_dense_matmul(W_1, x, adjoint_a=True,adjoint_b=True)
    z_1 = tf.nn.relu(a_1)
...
with tf.compat.v1.name_scope('Layer2'):
    ...
    a_2 = tf.sparse.sparse_dense_matmul(W_2, z_1, adjoint_a=True,adjoint_b=False)
    z_2 = tf.nn.relu(a_2)
...
with tf.compat.v1.name_scope('OutLayer'):
    ...
    logits_predict = tf.sparse.sparse_dense_matmul(w_out, z_2, adjoint_a=True,adjoint_b=False)
    logits_predict = tf.transpose(a=logits_predict)
...
with tf.compat.v1.name_scope('Training'):
    ...
    noise_update = lambda th: tf.random.normal(stddev=FLAGS.gdnoise, shape=tf.shape(input=th))

    add_gradient_op_list = [tf.compat.v1.assign_add(th, lr * mask_connected(th) * noise_update(th)) for th in theta_list]
    apply_l1_reg = [tf.compat.v1.assign_add(th, - lr * mask_connected(th) * FLAGS.l1) for th in theta_list]
...
with tf.compat.v1.name_scope('Stats'):
    ...
    get_non_zeros_sp = lambda w: tf.reduce_sum(input_tensor=tf.cast(tf.not_equal(w.values,0),dtype=tf.int32))
    ...
    global_connectivity = tf.reduce_sum(input_tensor=nb_entries) / tf.reduce_sum(input_tensor=sizes)
